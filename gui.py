...
class ConsoleApp:
    def __init__(self, root):
        ...
        # Output area
        self.output_area = scrolledtext.ScrolledText(self.frame, bg="black", fg="white", wrap=tk.WORD, height=15, font=("Courier", 20))
    ...
    def execute_command(self, event=None):
        ...
        if command.lower() == 'exit':
            ...
            self.show_current_directory()  # Get current directory from worker
        else:
            self.send_command_to_worker(command)
    ...
    def get_prompt(self):
        """ Generate the command prompt string based on username and current directory. """
        ...
        if relative_path.startswith(".."):
            ...
            return f"{self.bot_username}@{self.bot_username}:{self.current_directory}$ "
        else:
            # Show relative path if inside the home directory
            return f"{self.bot_username}@{self.bot_username}:~/{relative_path}$ "
    ...
    def get_current_directory_from_worker(self, url):
        ...
        try:
            ...
            return response.text.strip()  # Return the directory as text
